...
function HolidaysLabel() {
  const { isLoading, error, data } = useQuery("holidays", async () => {
    const req = await fetch(reqUrl);
    const data = await req.json();
    return data.filter((trip) => trip.acf.kategorie.name === "Wczasy");
  });
